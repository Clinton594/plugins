...
function uploadFile($FILE_CONSTRUCT, $directory, $allowed_files)
{
  $response = object(["status" => 0, "message" => ""]);
  if (!is_dir($directory)) {
    mkdir($directory, 0777, true);
  }
  $target_file = $directory . basename($FILE_CONSTRUCT["name"]);
  $FileType = strtolower(trim(pathinfo($target_file, PATHINFO_EXTENSION)));

  if (in_array($FileType, array_map("strtolower", $allowed_files))) {
    // Upload the file
    if (move_uploaded_file($FILE_CONSTRUCT["tmp_name"], $target_file)) {
      $response->status = 1;
      $response->data = $target_file;
    } else $response->message = "File not uploaded";
  } else $response->message = implode(", ", $allowed_files) . " are the only allowed files";
  return $response;
}
